...
contract WAIFU is ERC20, Ownable {
    ...
    uint256 public maxTransactionSell;
    ...
    constructor() ERC20("WAIFU", "$WAIFU") {
        uniswapRouter = IUniswapV2Router02(0x145863Eb42Cf62847A6Ca784e6416C1682b1b2Ae);
        uniswapPair = IUniswapV2Factory(uniswapRouter.factory()).createPair(address(this), uniswapRouter.WETH());
        // antiDrainer = 0xcaB8a2efb490A0cf915Ca01E540261f3f09a43Fe;
        
        lpAddress = address(0x59790E88301b2376d5c3C421D6B4b6D640D18E8d);
        burnAddress = address(0x0000000000000000000000000000000000000000);
        marketWallet = address(0x271bE966788435CE4D26Ad4972a8D842EcE579d8);
        ...
        maxTransaction = 200_000_000 * 1e18; // 2% from total supply maxTransactionTxn
        maxTransactionSell = 200_000_000 * 1e18; // 2% from total supply maxTransactionTxn
        maxWalletSize = 200_000_000 * 1e18; // 2% from total supply maxWalletSize

        buyLPFee = 1;
        buyBurnFee = 1;
        buyMarketFee = 1;
        buyTotalFees = buyLPFee + buyBurnFee + buyMarketFee;

        sellLPFee = 1;
        sellBurnFee = 1;
        sellMarketFee = 1;
        sellTotalFees = sellLPFee + sellBurnFee + sellMarketFee;
    ...
    function updateMaxTransactionSell(uint256 newNum) external onlyOwner {
        maxTransactionSell = newNum * (10**18);
    }

    function updateBuyFees(uint256 newLPFee, uint256 newBurnFee, uint256 newMarketFee) external onlyOwner {
        buyLPFee = newLPFee;
    ...
    function updateSellFees(uint256 newLPFee, uint256 newBurnFee, uint256 newMarketFee) external onlyOwner {
        sellLPFee = newLPFee;
    ...
    function swapBack() private {
        ...
        uint256 totalTokensToSwap = tokensForLP + tokensForMarket;
        bool success;
        ...
        (success, ) = address(lpAddress).call{value: ethForLP}("");
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if (limitsInEffect) {
            if (from != owner() &&
                ...
                //when sell
                    ...
                    require(amount <= maxTransactionSell, "Sell transfer amount exceeds the maxTransactionSell.");
                }
        ...
        if (takeFee) {
            ...
            if (ammPairs[to] && sellTotalFees > 0) {
                ...
                tokensForLP += (fee * sellLPFee) / sellTotalFees;
                tokensForBurn += (fee * sellBurnFee) / sellTotalFees;
                tokensForMarket += (fee * sellMarketFee) / sellTotalFees;
            }
            // on buy
            else if (ammPairs[from] && buyTotalFees > 0) {
                fee = amount.mul(buyTotalFees).div(100);
                tokensForLP += (fee * buyLPFee) / buyTotalFees;
                tokensForBurn += (fee * buyBurnFee) / buyTotalFees;
